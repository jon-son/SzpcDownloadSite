...
$upfile='../img/'.$_POST['upactivity']; //图片目录路径
$file=$_FILES['upimg'];
   ...
   if($_SERVER['REQUEST_METHOD']=='POST'){ //判断提交方式是否为POST
       //判断上传文件是否存在
        if(!is_uploaded_file($file['tmp_name'])){
            echo '<script language="JavaScript">;alert("图片不存在！");location.href="addactivity.html";</script>;';
            exit;
        }
       //判断文件大小是否大于500000字节
        if($file['size']>$max_size){
            echo '<script language="JavaScript">;alert("上传图片太大！");location.href="addactivity.html";</script>;';
            exit;
        }
       //判断图片文件的格式
        if(!in_array($file['type'],$arrType)){
            echo '<script language="JavaScript">;alert("上传图片格式错误！");location.href="addactivity.html";</script></script>;';
            exit;
         }
       // 判断存放文件目录是否存在
        ...
        if(!move_uploaded_file($file['tmp_name'],$picName)){
           echo '<script language="JavaScript">;alert("移动图片出错！");location.href="addactivity.html";</script>;';
           exit;
        }
        else{
	    $name=$_POST['upname'];
        ...
        $abstract=$_POST['upabstract'];
         $activity= $_POST['upactivity'];
        $img= iconv("GBK","UTF-8",$fname);
        ...
        {
        		unlink ($fname);
            die('<script language="JavaScript">;alert("信息提交失败，请重试！");location.href="addactivity.html";</script>;');exit;
        }
        ...
        $type="0";
        $sql="INSERT INTO activity(id,name,type,class,username,local,time,img,abstract)  VALUES('$row','$name',$type,'$activity','$username','$local','$time','$fname','$abstract');";
        ...
        if (!mysql_query($sql,$mysqlConn))
        {
            ...
            die('<script language="JavaScript">;alert("信息提交失败，请重试！");location.href="addactivity.html";</script>;');exit;
        	 ...
        	 echo '<script language="JavaScript">;alert("成功提交信息!");location.href="addactivity.html";</script>;';
