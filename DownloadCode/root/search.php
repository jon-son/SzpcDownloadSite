...
<html lang="en">
	...
	<body>
		...
		<div class="contents" align="center"></br>
		    ...
		    <form action="search.php?tablename=<?= $tablename ?>" method="post" class="search">
                <input type="search" autocomplete="off" placeholder="输入关键字"  name="search" required>
            </form>
            


			<div class="hot_download" style="border-radius: 15px;">
                ...
                <?php
					...
					$result=mysql_query($mysql->mysqlGetSearchList($tablename,$key));
			  		while($row=mysql_fetch_assoc($result))
                    {

                        $success = "review.php?state=success&tablename=$tablename&id=$row[id]";
                        $failed = "review.php?state=failed&tablename=$tablename&id=$row[id]";
                        if("$row[type]"=="0"){
                        ?>
                        ...
                        <div class="img_set">
                            <img class="img" src="../img/<?="$row[class]"?>/<?="$row[img]"?>" />
                        </div>
                        <div class="name_set">
                        ...
                        <div class="down_set">
                            ...
                            <a href="<?= $success?>" ><button class="review"> 审核通过 </button></a>
                            <a href="<?= $failed?>" ><button class="review"> 审核失败 </button></a>
					...
					mysql_query($mysql->mysqlSetCode);
					$result1=mysql_query($mysql->mysqlGetSearchList($tablename,$key));
                    
                    while($row=mysql_fetch_assoc($result1))
                    {
                    	$cancel = "review.php?state=cancel&tablename=$tablename&id=$row[id]";
                        $delete = "review.php?state=delete&tablename=$tablename&id=$row[id]";
                        ...
                        <div class="down_set">
                            <a href="<?="$row[path]"?>" ><input type="button" class="download" value="下载"></input></a></br></br>
                            <a href="<?= $cancel?>" ><button class="review"> 下架 </button></a>
                            <a href="<?= $delete?>" ><button class="review"> 删除 </button></a>
